...
(function (global) {
        ...
        <section id="main-menu-stage" class="w-full h-full flex flex-col justify-center items-center gap-3 sm:gap-4 py-3 overflow-y-auto shrink-0 custom-scrollbar">
            ...
            <div class="w-full max-w-sm sm:max-w-md bg-white border-2 border-emerald-200/90 rounded-[2rem] p-4 sm:p-5 shadow-sm relative overflow-hidden">
                <div class="space-y-1 text-xs sm:text-sm text-slate-600 font-medium">
                    <p><span class="text-slate-600" data-i18n="developed_by">${escapeHTML(i18n.t('developed_by'))}</span> <strong class="text-slate-800 font-black" data-i18n="author_name">${escapeHTML(i18n.t('author_name'))}</strong></p>
                    <p><span class="text-slate-600" data-i18n="phone_whatsapp">${escapeHTML(i18n.t('phone_whatsapp'))}</span> <strong class="text-slate-800 font-black tracking-wide" dir="ltr">01147992249</strong></p>
                </div>
                <hr class="my-2.5 border-slate-100">
                <div class="space-y-1 text-center">
                    <p class="text-xs sm:text-sm text-slate-600 font-bold leading-relaxed quran-font" data-i18n="dedication_prayer">${escapeHTML(i18n.t('dedication_prayer'))}</p>
                </div>
            </div>
            ...
            <div class="w-full max-w-sm sm:max-w-md px-4 py-2 bg-emerald-50/80 border border-emerald-200 rounded-2xl flex items-center justify-between shadow-inner">
                <label for="toggle-game-breaks" class="flex items-center gap-2 cursor-pointer text-xs sm:text-sm font-bold text-emerald-900 select-none">
                    <span aria-hidden="true">🎮</span>
                    <span data-i18n="enable_game_breaks">${escapeHTML(i18n.t('enable_game_breaks'))}</span>
                </label>
                <input type="checkbox" id="toggle-game-breaks" onchange="app.toggleGameBreaks(this.checked)" class="w-5 h-5 accent-emerald-600 rounded cursor-pointer" data-i18n-aria="enable_game_breaks" aria-label="${escapeHTML(i18n.t('enable_game_breaks'))}">
            </div>

            <!-- Menu Action Buttons -->
            <div class="grid grid-cols-1 gap-2.5 w-full max-w-sm sm:max-w-md px-2" role="group" data-i18n-aria="nav_section_aria" aria-label="${escapeHTML(i18n.t('nav_section_aria'))}">
                ${rulesButtonHtml}
                <button onclick="app.startChallenge()" class="bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-600 hover:to-teal-700 text-white font-black py-3.5 px-6 rounded-[1.5rem] text-lg shadow-[0_5px_0_#047857] active:translate-y-[5px] active:shadow-none transition-all flex items-center justify-between" data-i18n-aria="start_challenge" aria-label="${escapeHTML(i18n.t('start_challenge'))}">
                    <span class="flex items-center gap-2"><span aria-hidden="true">🚀</span> <span data-i18n="start_challenge">${escapeHTML(i18n.t('start_challenge'))}</span></span>
                    <span class="text-2xl" aria-hidden="true">➡</span>
                </button>
                <button onclick="app.jumpTo('ww_box')" class="bg-white border-4 border-amber-400 hover:bg-amber-50 text-amber-500 font-black py-3 px-6 rounded-[1.5rem] text-base shadow-[0_5px_0_#d97706] active:translate-y-[5px] active:shadow-none transition-all flex items-center justify-between" data-i18n-aria="open_word_games" aria-label="${escapeHTML(i18n.t('open_word_games'))}">
                    <span class="flex items-center gap-2"><span aria-hidden="true">🎡</span> <span data-i18n="open_word_games">${escapeHTML(i18n.t('open_word_games'))}</span></span>
                    <span class="text-2xl" aria-hidden="true">➡</span>
                </button>
            </div>
        </section>`;
    ...
    function renderLearningStage() {
        ...
        <section id="learning-stage" class="hidden w-full flex flex-col justify-between items-center gap-2 py-2 flex-1 overflow-hidden relative">
            ...
            <div id="session-drill-banner" class="hidden w-full max-w-2xl mx-auto px-4 py-2 bg-gradient-to-r from-rose-500 via-amber-500 to-rose-600 text-white rounded-2xl shadow-md flex items-center justify-between gap-3 shrink-0" role="region" data-i18n-aria="session_drill_title" aria-label="${escapeHTML(i18n.t('session_drill_title'))}" aria-live="polite">
                <div class="flex items-center gap-2 font-black text-xs sm:text-sm">
                    ...
                    <span id="session-drill-title" data-i18n="session_drill_title">${escapeHTML(i18n.t('session_drill_title'))}</span>
                </div>
                <div class="flex items-center gap-3">
                    ...
                    <button type="button" onclick="app.openEarlyExitModal()" class="w-7 h-7 bg-white/20 hover:bg-white/30 text-white rounded-full flex items-center justify-center font-black text-xs transition-colors cursor-pointer" data-i18n-aria="close" data-i18n-title="close" aria-label="${escapeHTML(i18n.t('close'))}" title="${escapeHTML(i18n.t('close'))}">✕</button>
                </div>
            </div>

            <div class="flex-1 flex items-center justify-center w-full px-2 py-1 overflow-hidden" id="word-display-area" role="region" data-i18n-aria="aria_current_word" aria-label="${escapeHTML(i18n.t('aria_current_word'))}" aria-live="polite" aria-atomic="true"></div>

            <div class="w-full max-w-md sm:max-w-lg space-y-2 shrink-0 pb-3 px-2">
                <div class="grid grid-cols-2 gap-3 sm:gap-4">
                    <button onclick="app.evaluate(false)" class="bg-rose-500 hover:bg-rose-600 text-white py-3.5 sm:py-4 rounded-[1.75rem] text-lg sm:text-xl shadow-[0_5px_0_#be123c] active:translate-y-[5px] active:shadow-none transition-all flex items-center justify-center gap-2 font-black" data-i18n-aria="mark_incorrect_aria" aria-label="${escapeHTML(i18n.t('mark_incorrect_aria'))}">
                        <span aria-hidden="true">❌</span> <span data-i18n="btn_incorrect">${escapeHTML(i18n.t('btn_incorrect'))}</span>
                    </button>
                    <button onclick="app.evaluate(true)" class="bg-emerald-500 hover:bg-emerald-600 text-white py-3.5 sm:py-4 rounded-[1.75rem] text-lg sm:text-xl shadow-[0_5px_0_#047857] active:translate-y-[5px] active:shadow-none transition-all flex items-center justify-center gap-2 font-black" data-i18n-aria="mark_correct_aria" aria-label="${escapeHTML(i18n.t('mark_correct_aria'))}">
                        <span aria-hidden="true">✔</span> <span data-i18n="btn_correct">${escapeHTML(i18n.t('btn_correct'))}</span>
                    </button>
                </div>
                <div class="flex justify-center pt-1">
                    <button onclick="app.prev()" class="bg-slate-200/90 hover:bg-slate-300 text-slate-600 py-1.5 px-5 rounded-full text-xs font-bold transition-colors shadow-sm flex items-center gap-1.5" data-i18n-aria="prev_card_aria" aria-label="${escapeHTML(i18n.t('prev_card_aria'))}">
                        <span aria-hidden="true">⬅</span> <span data-i18n="btn_prev">${escapeHTML(i18n.t('btn_prev', 'السابق'))}</span>
                    </button>
                </div>
            </div>
        </section>`;
    ...
    function renderWordwallPlayroom(wordwallTabsHtml) {
        ...
        <section id="wordwall-stage" class="hidden w-full h-full flex flex-col items-center py-3 px-2 overflow-hidden bg-slate-50 rounded-3xl border border-slate-200 shadow-inner">
            <div class="w-full max-w-4xl flex flex-col sm:flex-row justify-between items-center mb-3 shrink-0 gap-3 px-3">
                <div class="flex items-center gap-2.5 flex-wrap">
                    <h2 class="text-xl sm:text-2xl font-black text-emerald-600 drop-shadow-sm flex items-center gap-2">
                        <span aria-hidden="true">🎡</span> <span data-i18n="games_room_title">${escapeHTML(i18n.t('games_room_title'))}</span>
                    </h2>
                    <button type="button" id="btn-ww-mistakes-filter" onclick="if(typeof wordwallRoom !== 'undefined' && wordwallRoom.toggleMistakesFilter) wordwallRoom.toggleMistakesFilter();" class="px-3 py-1 bg-amber-100 hover:bg-amber-200 text-amber-900 border border-amber-300 rounded-full font-bold text-xs shadow-xs transition-all flex items-center gap-1.5 cursor-pointer" data-i18n-aria="aria_ww_filter_mistakes" data-i18n-title="ww_filter_session_mistakes" aria-label="${escapeHTML(i18n.t('aria_ww_filter_mistakes'))}" title="${escapeHTML(i18n.t('ww_filter_session_mistakes'))}">
                        <span aria-hidden="true">🎯</span>
                        <span id="ww-filter-text" data-i18n="ww_filter_session_mistakes">${escapeHTML(i18n.t('ww_filter_session_mistakes'))}</span>
                    </button>
                </div>
            ...
            <div class="flex-1 w-full flex flex-col items-center justify-center overflow-hidden relative min-h-0">
                <div id="ww-box-container" role="tabpanel" aria-labelledby="tab-box" class="w-full h-full flex flex-col items-center min-h-0 overflow-hidden">
                    <div id="box-prompt-text" class="bg-indigo-100 text-indigo-800 px-3.5 py-1 rounded-full text-xs font-bold mb-2 shrink-0 shadow-sm border border-indigo-200" data-i18n="prompt_box">
                        ${escapeHTML(i18n.t('prompt_box'))}
                    </div>
                ...
                <div id="ww-ladder-container" role="tabpanel" aria-labelledby="tab-ladder" class="hidden w-full h-full flex flex-col items-center justify-between p-2 max-w-4xl mx-auto overflow-y-auto custom-scrollbar" data-i18n-aria="aria_ladder_game" aria-label="${escapeHTML(i18n.t('aria_ladder_game'))}">
                    ...
                    <div class="w-full flex justify-between items-center bg-white/80 border border-slate-200 rounded-2xl px-4 py-2 shrink-0 shadow-sm">
                        <div class="flex items-center gap-2">
                            ...
                            <button id="ladder-btn-5" onclick="ladderGame.setTarget(5)" class="py-1 px-3.5 rounded-full font-bold text-xs bg-emerald-500 text-white shadow-sm transition-all cursor-pointer" data-i18n-aria="ladder_target_5_aria" aria-label="${escapeHTML(i18n.t('ladder_target_5_aria'))}">5 <span data-i18n="ladder_steps_unit">${escapeHTML(i18n.t('ladder_steps_unit'))}</span></button>
                            <button id="ladder-btn-10" onclick="ladderGame.setTarget(10)" class="py-1 px-3.5 rounded-full font-bold text-xs bg-slate-100 text-slate-700 hover:bg-slate-200 transition-all cursor-pointer" data-i18n-aria="ladder_target_10_aria" aria-label="${escapeHTML(i18n.t('ladder_target_10_aria'))}">10 <span data-i18n="ladder_steps_unit">${escapeHTML(i18n.t('ladder_steps_unit'))}</span></button>
                        </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-12 gap-3 w-full flex-1 my-2 min-h-0 items-center">
                        <div class="md:col-span-4 flex flex-col gap-1.5 w-full max-h-[220px] md:max-h-[300px] overflow-y-auto custom-scrollbar p-2 bg-slate-100/80 rounded-2xl border border-slate-200" id="ladder-rungs" role="region" data-i18n-aria="aria_ladder_steps" aria-label="${escapeHTML(i18n.t('aria_ladder_steps'))}"></div>
                        <div class="md:col-span-8 flex flex-col justify-center items-center bg-white border-4 border-emerald-400 rounded-3xl shadow-xl p-4 min-h-[150px] md:min-h-[220px] w-full cursor-pointer select-none hover:border-emerald-500 transition-all group" onclick="if(typeof ladderGame !== 'undefined') ladderGame.grade(true);" role="button" tabindex="0" onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();if(typeof ladderGame!=='undefined')ladderGame.grade(true);}" title="اضغط للارتقاء">
                            <div id="ladder-word-display" class="w-full flex-1 flex items-center justify-center text-center pointer-events-none" role="region" data-i18n-aria="aria_ladder_challenge_word" aria-label="${escapeHTML(i18n.t('aria_ladder_challenge_word'))}" aria-live="polite"></div>
                            <span class="text-[11px] text-slate-400 mt-2 font-bold group-hover:text-emerald-600 transition-colors pointer-events-none">اضغط على الكلمة أو الدرجة للارتقاء 🪜</span>
                        </div>
                    </div>
                    <div class="w-full max-w-md shrink-0 space-y-2 pb-1 sticky bottom-0 bg-white/95 backdrop-blur-xs p-2 rounded-2xl border border-slate-200/80 shadow-md z-10">
                        <div class="grid grid-cols-2 gap-3">
                            <button onclick="ladderGame.grade(false)" class="bg-rose-500 hover:bg-rose-600 text-white py-3 rounded-2xl text-base font-black shadow-[0_4px_0_#be123c] active:translate-y-1 active:shadow-none transition-all flex items-center justify-center gap-2 cursor-pointer" data-i18n-aria="aria_step_down" aria-label="${escapeHTML(i18n.t('aria_step_down'))}">
                                <span aria-hidden="true">❌</span> <span data-i18n="btn_incorrect">${escapeHTML(i18n.t('btn_incorrect'))}</span> (1)
                            </button>
                            <button onclick="ladderGame.grade(true)" class="bg-emerald-500 hover:bg-emerald-600 text-white py-3 rounded-2xl text-base font-black shadow-[0_4px_0_#047857] active:translate-y-1 active:shadow-none transition-all flex items-center justify-center gap-2 cursor-pointer" data-i18n-aria="aria_step_up" aria-label="${escapeHTML(i18n.t('aria_step_up'))}">
                                <span aria-hidden="true">✔</span> <span data-i18n="btn_correct">${escapeHTML(i18n.t('btn_correct'))}</span> (2)
                            </button>
                        </div>
                ...
                <div id="ww-wheel-container" role="tabpanel" aria-labelledby="tab-wheel" class="hidden w-full h-full flex flex-col justify-center items-center gap-4" data-i18n-aria="game_wheel_tab" aria-label="${escapeHTML(i18n.t('game_wheel_tab'))}">
                    <div class="bg-amber-100 text-amber-800 px-3.5 py-1 rounded-full text-xs font-bold shrink-0 shadow-sm border border-amber-200" id="wheel-status" role="status" aria-live="polite" data-i18n="wheel_tap_spin">
                        ${escapeHTML(i18n.t('wheel_tap_spin'))}
                    </div>
                    <div class="relative w-[80vw] max-w-[340px] aspect-square flex items-center justify-center shrink-0 drop-shadow-2xl">
                        <canvas id="wheel-canvas" width="400" height="400" class="w-full h-full object-contain" role="img" data-i18n-aria="aria_wheel_canvas" aria-label="${escapeHTML(i18n.t('aria_wheel_canvas'))}"></canvas>
                ...
                <div id="ww-cards-container" role="tabpanel" aria-labelledby="tab-cards" class="hidden w-full h-full flex flex-col justify-center items-center gap-5" data-i18n-aria="game_cards_tab" aria-label="${escapeHTML(i18n.t('game_cards_tab'))}">
                    ...
                    <div class="card-pile flex justify-center items-center shrink-0 mt-2">
                        ...
                        <button type="button" onclick="cardsGame.dealNextCard()" id="active-deck-card" class="stacked-card bg-gradient-to-br from-rose-500 to-pink-600 border-4 border-white flex flex-col items-center justify-center text-white cursor-pointer hover:-translate-y-6 hover:rotate-2 transition-all duration-300 z-10 shadow-2xl" data-i18n-aria="aria_deal_card" aria-label="${escapeHTML(i18n.t('aria_deal_card'))}">
                            <span class="text-6xl drop-shadow-md" aria-hidden="true">🃏</span>
                            <span class="font-black mt-4 text-xs tracking-[0.2em] bg-black/20 px-3.5 py-1 rounded-full" data-i18n="deal_card_btn">${escapeHTML(i18n.t('deal_card_btn'))}</span>
                        </button>
                    </div>
                </div>

                <!-- 3D FLIP TILES CONTAINER -->
                <div id="ww-tiles-container" role="tabpanel" aria-labelledby="tab-tiles" class="hidden w-full h-full flex flex-col items-center min-h-0 overflow-hidden" data-i18n-aria="aria_tiles_game" aria-label="${escapeHTML(i18n.t('aria_tiles_game'))}">
                    <div class="w-full flex flex-col sm:flex-row justify-between items-center bg-white/90 border border-slate-200 rounded-2xl px-4 py-2 mb-2 shrink-0 shadow-sm max-w-5xl gap-2">
                        <div id="tiles-prompt-text" class="bg-indigo-100 text-indigo-900 px-3.5 py-1 rounded-full text-xs font-black shadow-xs border border-indigo-200" data-i18n="prompt_tiles">
                            ${escapeHTML(i18n.t('prompt_tiles'))}
                        </div>
                        <div class="flex items-center gap-2">
                            <button onclick="tilesGame.flipAll()" class="bg-slate-100 hover:bg-slate-200 text-slate-800 font-black px-3.5 py-1 rounded-full text-xs shadow-xs transition-colors" data-i18n-aria="aria_flip_all_tiles" aria-label="${escapeHTML(i18n.t('aria_flip_all_tiles'))}"><span aria-hidden="true">🔄</span> <span data-i18n="flip_all_tiles">${escapeHTML(i18n.t('flip_all_tiles'))}</span></button>
                            <button onclick="tilesGame.reset()" class="bg-amber-100 hover:bg-amber-200 text-amber-900 font-black px-3.5 py-1 rounded-full text-xs shadow-xs transition-colors" data-i18n-aria="aria_reset_tiles" aria-label="${escapeHTML(i18n.t('aria_reset_tiles'))}"><span aria-hidden="true">⚡</span> <span data-i18n="reset_tiles">${escapeHTML(i18n.t('reset_tiles'))}</span></button>
                        </div>
                    </div>
                ...
                <div id="ww-honeycomb-container" role="tabpanel" aria-labelledby="tab-honeycomb" class="hidden w-full h-full flex flex-col items-center min-h-0 overflow-hidden" data-i18n-aria="aria_honeycomb_game" aria-label="${escapeHTML(i18n.t('aria_honeycomb_game'))}">
                    <div class="w-full flex justify-between items-center bg-white/90 border border-slate-200 rounded-2xl px-4 py-2 mb-2 shrink-0 shadow-sm max-w-5xl">
                        <div class="flex items-center gap-2">
                            <span class="text-amber-600 font-black text-sm"><span aria-hidden="true">⬡</span> <span data-i18n="honeycomb_map_title">${escapeHTML(i18n.t('honeycomb_map_title'))}</span></span>
                        ...
                        <button onclick="honeycombGame.reset()" class="bg-amber-100 hover:bg-amber-200 text-amber-900 font-black px-3.5 py-1 rounded-full text-xs shadow-xs transition-colors" data-i18n-aria="aria_reset_honeycomb" aria-label="${escapeHTML(i18n.t('aria_reset_honeycomb'))}"><span aria-hidden="true">🔄</span> <span data-i18n="reset_honeycomb">${escapeHTML(i18n.t('reset_honeycomb'))}</span></button>
                    </div>
            ...
            <div class="flex justify-center w-full mt-2 shrink-0 pt-2 border-t border-slate-200">
                <button onclick="app.finishToSummary()" class="bg-slate-800 hover:bg-slate-900 text-white px-6 py-2.5 rounded-full font-black text-xs sm:text-sm shadow-md transition-transform hover:-translate-y-0.5 flex items-center gap-2" data-i18n-aria="view_final_report_aria" aria-label="${escapeHTML(i18n.t('view_final_report_aria'))}">
    ...
    function renderModals() {
        ...
        <div id="word-overlay" class="hidden fixed inset-0 bg-slate-900/80 z-50 flex flex-col justify-center items-center p-3 sm:p-6 backdrop-blur-md" role="dialog" aria-modal="true" aria-labelledby="revealed-info">
            <div class="giant-word-overlay bg-white rounded-[2.5rem] w-full max-w-3xl max-h-[90vh] p-5 sm:p-8 flex flex-col justify-between items-center shadow-2xl border-4 border-emerald-400 overflow-y-auto custom-scrollbar">
                <div class="w-full flex justify-between items-center px-2 shrink-0 mb-3">
                    <span id="revealed-info" class="bg-emerald-100 text-emerald-800 px-3 py-1 rounded-full font-bold text-xs uppercase tracking-wider" data-i18n="question_card_badge">${escapeHTML(i18n.t('question_card_badge'))}</span>
                    <button onclick="app.closeOverlay()" class="bg-slate-100 text-rose-700 hover:bg-rose-100 hover:text-rose-600 px-3.5 py-1 rounded-full text-xs font-black transition-colors" data-i18n-aria="close" aria-label="${escapeHTML(i18n.t('close'))}">✕ <span data-i18n="close">${escapeHTML(i18n.t('close'))}</span></button>
                </div>
                <div id="giant-arabic-word" class="flex-1 flex items-center justify-center w-full py-4 min-h-[160px]" role="region" data-i18n-aria="aria_revealed_word" aria-label="${escapeHTML(i18n.t('aria_revealed_word'))}" aria-live="polite">
                </div>
                <div class="w-full max-w-sm shrink-0 mt-4">
                    <div class="grid grid-cols-2 gap-3">
                        <button onclick="app.gradeResult(false)" class="bg-rose-500 hover:bg-rose-600 text-white py-3.5 rounded-2xl text-base shadow-[0_4px_0_#be123c] active:translate-y-1 active:shadow-none transition-all flex items-center justify-center gap-2 font-black" data-i18n-aria="aria_mark_missed" aria-label="${escapeHTML(i18n.t('aria_mark_missed'))}">
                            <span aria-hidden="true">❌</span> <span data-i18n="btn_incorrect">${escapeHTML(i18n.t('btn_incorrect'))}</span>
                        </button>
                        <button onclick="app.gradeResult(true)" class="bg-emerald-500 hover:bg-emerald-600 text-white py-3.5 rounded-2xl text-base shadow-[0_4px_0_#047857] active:translate-y-1 active:shadow-none transition-all flex items-center justify-center gap-2 font-black" data-i18n-aria="aria_mark_perfect" aria-label="${escapeHTML(i18n.t('aria_mark_perfect'))}">
                            <span aria-hidden="true">✔</span> <span data-i18n="txt_perfect">${escapeHTML(i18n.t('txt_perfect'))}</span>
                        </button>
                    </div>
                </div>
            </div>
        ...
        <div id="early-exit-modal" class="hidden fixed inset-0 bg-slate-900/80 z-50 flex flex-col justify-center items-center p-4 backdrop-blur-md" role="dialog" aria-modal="true" aria-labelledby="early-exit-title">
            <div class="bg-white rounded-[2.5rem] w-full max-w-md p-6 sm:p-7 flex flex-col items-center text-center shadow-2xl border-4 border-rose-200 animate-in fade-in zoom-in duration-150" dir="${(typeof i18n !== 'undefined' && i18n.getActiveMeta) ? i18n.getActiveMeta().dir : 'rtl'}">
                ...
                <h3 id="early-exit-title" class="text-xl sm:text-2xl font-black text-slate-800 mb-2" data-i18n="early_exit_title">
                    ${escapeHTML(i18n.t('early_exit_title'))}
                </h3>
                <p id="early-exit-desc" class="text-slate-600 font-bold text-xs sm:text-sm leading-relaxed mb-6" data-i18n="early_exit_desc">
                    ${escapeHTML(i18n.t('early_exit_desc'))}
                </p>
                <div class="w-full flex flex-col gap-2.5">
                    <button type="button" id="btn-early-exit-save" onclick="app.confirmEarlyExit('save')" class="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-black py-3 px-4 rounded-2xl text-sm shadow-[0_3px_0_#047857] active:translate-y-0.5 active:shadow-none transition-all flex items-center justify-center gap-2 cursor-pointer">
                        <span aria-hidden="true">💾</span> <span data-i18n="early_exit_save_btn">${escapeHTML(i18n.t('early_exit_save_btn'))}</span>
                    </button>
                    <button type="button" id="btn-early-exit-cancel" onclick="app.confirmEarlyExit('cancel')" class="w-full bg-rose-50 hover:bg-rose-100 text-rose-700 border border-rose-300 font-black py-3 px-4 rounded-2xl text-sm transition-colors flex items-center justify-center gap-2 cursor-pointer">
                        <span aria-hidden="true">🔄</span> <span data-i18n="early_exit_cancel_btn">${escapeHTML(i18n.t('early_exit_cancel_btn'))}</span>
                    </button>
                    <button type="button" id="btn-early-exit-resume" onclick="app.closeEarlyExitModal()" class="w-full bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold py-2.5 px-4 rounded-2xl text-xs transition-colors cursor-pointer mt-1" data-i18n="early_exit_resume_btn">
                        ${escapeHTML(i18n.t('early_exit_resume_btn'))}
                    </button>
                </div>
            </div>
        </div>`;
    ...
    function renderSummaryScreen() {
        ...
        <section id="summary-screen" class="hidden w-full max-w-2xl bg-white rounded-[2.5rem] p-6 sm:p-8 shadow-2xl text-center space-y-5 border-4 border-emerald-100 shrink-0 my-auto z-10" role="region" data-i18n-aria="aria_final_summary" aria-label="${escapeHTML(i18n.t('aria_final_summary'))}">
            ...
            <div class="grid grid-cols-2 gap-4 items-center">
                <div class="space-y-2">
                    <span class="text-slate-600 font-bold uppercase text-xs sm:text-sm tracking-wider" data-i18n="points_gathered_label">${escapeHTML(i18n.t('points_gathered_label'))}</span>
                ...
                <div class="flex flex-col items-center">
                    <div class="chart-container"><canvas id="summaryChart" role="img" data-i18n-aria="aria_chart_summary" aria-label="${escapeHTML(i18n.t('aria_chart_summary'))}"></canvas></div>
                    <p id="summary-chart-text" class="mt-2 text-sm font-bold text-slate-600" aria-live="polite"></p>
                </div>
            </div>
            <div class="flex flex-wrap justify-center pt-2 gap-3">
                <button type="button" id="btn-review-mistakes" onclick="app.startSessionDrill()" class="hidden bg-rose-500 hover:bg-rose-600 text-white text-base font-bold py-3 px-8 rounded-full shadow-lg transition-all flex items-center gap-2 cursor-pointer" data-i18n-aria="aria_review_mistakes" data-i18n-title="review_mistakes_btn" aria-label="${escapeHTML(i18n.t('aria_review_mistakes'))}" title="${escapeHTML(i18n.t('review_mistakes_btn'))}">
                    <span data-i18n="review_mistakes_btn">${escapeHTML(i18n.t('review_mistakes_btn'))}</span> <span aria-hidden="true">🎯</span>
                </button>
